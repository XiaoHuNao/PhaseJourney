...
public class RecipePhaseManager extends PhaseManager<RecipeLockContext> {
    ...
    public boolean isRestricted(Level level, BlockPos pos, Player player, RecipeType<?> recipeType, ResourceLocation recipeID) {
        for (PhaseType phaseType : PhaseType.values()) {
            for (Pair<ResourceLocation, RecipeLockContext> pair : phaseContexts.get(phaseType)) {
                ResourceLocation phase = pair.getFirst();
                RecipeLockContext ctx = pair.getSecond();

                if (!ctx.getSupportedPhaseTypes().contains(phaseType)) {
                    continue;
                }

                if (recipeType != ctx.recipeType) {
                    continue;
                }

                if (!recipeID.equals(ctx.recipeID)) {
                    continue;
                }
